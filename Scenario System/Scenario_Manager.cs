...
//시나리오 진행도, 퀘스트아이템 획득여부, 시나리오 진행 함수를 포함하는 스크립트
public class Scenario_Manager : MonoBehaviour
{
    public System.Single scenario_main_number = 0;  //큰 줄기의 시나리오
    public System.Single scenario_sub_number = 0;   //작은 줄기의 시나리오
    ...
    {
        //퀘스트 클리어시 지정 장소에 도착하면 시나리오 진행
        //하지만 원래부터 그 장소에 있었다면 Trigger Enter가 정상 작동 하지 않으니 껏다 켰다를 반복
        while (true)
        {
    ...
    //타워레벨과 플레이어레벨등을 고려해서 다음 층의 입장가능 여부를 판단.
    ...
    public void check_towerLevel_ForLevel() 
    {
    ...
    //NPC관련 스크립팅
    public void FollowNpc(string targetModelName) //이벤트로 NPC가 동행하게 됨.
    {
        ...
        if(targetModelName != "luna")
        {
            if(targetModelName == "model4_school") // 내부 if의 조건은 non씬에서 대려갈때 이용.
            {
    ...
    public void NotFollowNpc(string targetModelName) //이벤트혹은 이벤트 종료로 NPC를 더이상 동행하지 않게함
    {
        GameObject targetNpc = GameObject.Find(targetModelName);
   ...
   public void Wave6_1()
    ...
    private void OnEnable() //Lua코드 적용
    {
